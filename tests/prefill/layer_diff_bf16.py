...
# -*- coding: utf-8 -*-
"""
Export a debug prefill graph (BF16) that surfaces per-layer hidden states
and compare its ExecuTorch outputs against the PyTorch wrapper. This mirrors
layer_diff.py but keeps everything in bfloat16 to diagnose BF16 exports.
"""
...
def maybe_load(path: Optional[str]) -> Optional[torch.Tensor]:
    ...
    return torch.from_numpy(np.load(path)).to(torch.bfloat16).contiguous()


# ----------------- HF modules -----------------
def load_text_only_bf16(model_id: str, revision: str):
...
class PrefillHiddenWrapper(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, audio_ctx: torch.Tensor):
        ...
        hidden = torch.cat([tokens, audio_ctx.to(torch.bfloat16).unsqueeze(0)], dim=1)
...
def export_debug_prefill(prefill: nn.Module, l_text: int, n_audio: int, out_path: str):
    ...
    audio_ctx_ex = torch.zeros(n_audio, 2048, dtype=torch.bfloat16)
...
def _main_impl():
    ...
    ap.add_argument("--pte_out", default="llm_prefill_tokens_debug_bf16.pte")
    ...
    lm_backbone, lm_head, embed = load_text_only_bf16(args.model_id, args.revision)
    gc.collect()
