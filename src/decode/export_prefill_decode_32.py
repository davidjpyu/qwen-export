...
def load_text_only_fp32(model_id: str, revision: str):
    ...
    lm_backbone.float(); lm_head.float(); embed.float()
...
class PrefillTokensWrapper(nn.Module):
    ...
    def forward(self, input_ids:torch.Tensor, audio_ctx:torch.Tensor):
        ...
        hidden = torch.cat([x, audio_ctx.to(torch.float32).unsqueeze(0)], dim=1)
...
def build_fixed_capacity_kv_examples(prefill_mod: nn.Module, cache_T: int) -> List[torch.Tensor]:
    with torch.no_grad():
        ...
        aud_min = torch.zeros(1,2048,dtype=torch.float32)
...
def main():
    ...
    ap.add_argument("--prefill_out", default="llm_prefill_tokens_64p_750a_fp32.pte")
    ap.add_argument("--decode_out_tmpl", default="llm_decode_cacheT{cache_T}_fp32.pte")
    args = ap.parse_args()

    print(f"[INFO] load {args.model_id} (revision={args.revision}) in fp32...")
    lm_backbone, lm_head, embed = load_text_only_fp32(args.model_id, args.revision); gc.collect()
    print("[INFO] model ready.")

    # prefill
    print("[INFO] tracing prefill path...")
    input_ids_ex = torch.ones(1,args.l_text,dtype=torch.int32)
    audio_ctx_ex = torch.zeros(args.n_audio,2048,dtype=torch.float32)
    ...
    # decode
    print("[INFO] tracing decode path...")
    ...
    past_ex   = torch.tensor([0], dtype=torch.int64)
